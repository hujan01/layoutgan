...
class WifeDiscriminator(nn.Module):
    ...
    def forward(self, x_in):
        ...
        x = self.activatation(self.conv3_bn(self.conv3(x)))

        x = x.view(x.size(0), -1)
        x = self.activatation(self.fc1(x))
        x = self.fc2(x)
...
class RelationDiscriminator(nn.Module):
    ...
    def __init__(self, batch_size, geo_num, cls_num, num_elements):
        ...
        self.encoder_bn1 = nn.LayerNorm((num_elements, self.feature_size*2))   
        self.encoder_fc2 = nn.Linear(self.feature_size*2, self.feature_size*2*2)
        self.encoder_bn2 = nn.LayerNorm((num_elements, self.feature_size*2*2)) 
    ...
    def forward(self, x_in):
        ...
        x = torch.sigmoid(self.decoder_fc5(x))
        return x
