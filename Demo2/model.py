...
class Generator(nn.Module):
    ...
    def __init__(self, num_elements, geo_num, cls_num): 
        ...
        self.encoder_fc1 = nn.Linear(self.feature_size, 64)
        #self.encoder_bn1 = nn.BatchNorm1d(num_elements)  
        self.encoder_fc2 = nn.Linear(64, 128)
        #self.encoder_bn2 = nn.BatchNorm1d(num_elements)
        self.encoder_fc3 = nn.Linear(128, 1024)

        #stacked relation 
        self.attention_1 = Attention(1024, generate=False)
        self.attention_2 = Attention(1024, generate=False)
        self.attention_3 = Attention(1024, generate=False)
        self.attention_4 = Attention(1024, generate=False)

        #Decoder
        self.decoder_fc4 = nn.Linear(1024, 128)
        self.decoder_bn4 = nn.BatchNorm1d(num_elements)
        self.decoder_fc5 = nn.Linear(128, 64)
        self.decoder_bn5 = nn.BatchNorm1d(num_elements)
        self.decoder_fc8 = nn.Linear(64,self.feature_size)
    ...
    def forward(self, x):
        ...
        x = self.activatation(self.encoder_fc1(x))
        ...
        out = self.activatation(self.decoder_bn5(self.decoder_fc5(out)))
        out = self.activatation(self.decoder_fc8(out))
...
class RelationDiscriminator(nn.Module):
    ...
    def __init__(self, batch_size, geo_num, cls_num, num_elements):
        ...
        # relation
        self.attention= Attention(1024, generate=False)
        
        #max-pooling 用于进行全局
        self.g = nn.MaxPool1d(kernel_size=num_elements)

        # Decode
        self.decoder_fc4 = nn.Linear(1024, 512)
        self.decoder_fc5 = nn.Linear(512,256)
        self.decoder_fc6 = nn.Linear(256,1)
    ...
    def forward(self, x_in):
        ...
        x = self.activatation(self.decoder_fc6(x))
        x = torch.sigmoid(x)
        return x