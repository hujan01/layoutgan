...
#         nn.init.normal_(self.W.weight, 0, 0.02)


#     def forward(self, x): #x: (256, 128, 12)
#         element_num = x.size(1) #批次大小
#         batch_res = []
#         for bdx, batch in enumerate(x):
#             f_prime = []
#             for idx, i in enumerate(batch):
#                 self_attention = torch.Tensor(torch.zeros(i.size(0)))
#                 self_attention = self_attention.cuda()
#                 for jdx, j in enumerate(batch):
#                     if idx == jdx:
#                         continue
#                     u = self.activation(self.g(j))
#                     iv = self.activation(self.theta(i))
#                     jv = self.activation(self.phi(j))
#                     dot = torch.matmul(iv.t(), jv).view(1)
#                     H1 = dot*u
#                     self_attention += H1.cuda()
#                 if self.generate: 
#                     f_prime.append(self.activation(self.W(self_attention / element_num)) + i)
#                 else:
#                     f_prime.append(self.activation(self.W(self_attention / element_num)))
